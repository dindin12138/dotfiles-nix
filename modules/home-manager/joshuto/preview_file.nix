{ pkgs, ... }:
{
  preview_file = pkgs.writeShellScriptBin "preview_file" ''
    IFS=$'\n'
    ...
    handle_extension() {
    	...
    	a | ace | alz | arc | arj | bz | bz2 | cab | cpio | deb | gz | jar | lha | lz | lzh | lzma | lzo | \
    		...
    		${pkgs.atool}/bin/atool --list -- "''${FILE_PATH}" && exit 0
    		bsdtar --list --file "''${FILE_PATH}" && exit 0
    		exit 1
    		;;
    	rar)
    		## Avoid password prompt by providing empty password
    		${pkgs.unrar}/bin/unrar lt -p- -- "''${FILE_PATH}" && exit 0
    		exit 1
    		;;
    	7z)
    		## Avoid password prompt by providing empty password
    		${pkgs.p7zip}/bin/7z l -p -- "''${FILE_PATH}" && exit 0
    		exit 1
    		;;
    	...
    	htm | html | xhtml)
    		## Preview as text conversion
    		${pkgs.w3m}/bin/w3m -dump "''${FILE_PATH}" && exit 0
    		...
    		## JSON
    	json | ipynb)
    		${pkgs.jq}/bin/jq --color-output . "''${FILE_PATH}" && exit 0
